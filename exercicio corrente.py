...
def resp_df(dados):
    ...
    fa = ['ALMOX', 'ALMO', 'ALMX', 'GARAGEM', 'VIATURA', 'MECANICA', 'VEICULO', 'VEICULOS', 'ECT', 'CORREIOS',
          ...
          'FORTALEZA SERVICOS EMPRESARIAIS EIRELI', 'GCALC', 'DGO', 'COTER', 'PALESTRA ECEME', 'DIÁRIAS']
    ...
    for texto, teste in dados.iterrows():
        if 'CITEX' in teste['OBS'][:10]:
            if any(x in str(teste['NOME_FAV']) for x in fa):
                resp.append('FA')
                continue
            if any(x in str(teste['NOME_FAV']) for x in dgsi):
                resp.append('DGSI')
                continue
            if any(x in str(teste['NOME_FAV']) for x in salc):
                resp.append('SALC')
                continue
            if any(x in str(teste['NOME_FAV']) for x in dci):
                resp.append('DCI')
                continue
        ...
        if any(x in str(teste['NOME_FAV']) for x in dci):
            resp.append('DCI')
            continue
        if any(x in str(teste['NOME_FAV']) for x in cta):
            resp.append('7ºCTA')
            continue
...
dados.to_csv('saldo_exercicio_corrente.csv')
